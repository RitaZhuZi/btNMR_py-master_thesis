...
class Generator():
    ...
    def amide_proton_parameters(self):
        ...
        self.phase_list = tf.math.abs(tf.random.normal(shape=(self.resi_num,1), mean=(self.phase_range[0] + self.phase_range[1]) / 2, stddev=self.phase_range[1] / 4, dtype=tf.float32))   #Random phase for each spin
        self.phase_list = tf.cast(self.phase_list, dtype=tf.complex64)
